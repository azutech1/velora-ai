  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      {QUICK_SWAP_PAIRS.map(([from, to]) => (
        <button key={`${from}-${to}`} onClick={() => onPairSelect(from, to)} className="rounded-lg border border-white/10 bg-white/[0.04] p-4 text-left transition hover:-translate-y-1 hover:border-cyan/35 hover:bg-cyan/10">
