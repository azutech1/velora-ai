...
export const ASSISTANT_KNOWLEDGE: AssistantKnowledgeEntry[] = [
  ...
  },
  {
    ...
    answer:
      "Liquidity pools let users deposit paired tokens into a pool so swaps can route through shared liquidity. In Velora AI Testnet Beta, the active designed pool focus is USDC/EURC on Arc Testnet. Liquidity Pools are coming soon on Arc Testnet, and Velora intentionally does not simulate fake liquidity transactions. Real Add/Remove Liquidity will be enabled only after official Arc infrastructure is available. USDC/USDT and EURC/USDT are visible as Coming Soon. Pool values, rewards, and positions are for testing only and do not represent real yield or financial returns.",
